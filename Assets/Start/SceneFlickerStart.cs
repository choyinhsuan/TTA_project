...
public class StartFlickerStart : MonoBehaviour
{
    public CanvasGroup canvasGroup;             // UI 黑幕
    public Image flickerCenterImage;            // 閃爍圖片
    public float fadeDuration = 2f;             // 淡入、淡出時間
    public float endFadeStartTime = 55f;        // 幾秒後畫面再暗下來
    ...
    IEnumerator FlickerSequence()
    {
        // 初始畫面黑
        canvasGroup.alpha = 1;
        flickerCenterImage.enabled = false;

        // 閃爍幾次
        for (int i = 0; i < 2; i++)
        {
            float waitTime = Random.Range(0.1f, 0.3f);
            flickerCenterImage.enabled = true;
